...
# směrové textury atd.).
#
# Tento skript přidává proměnnou `bullet_scene_override`, která umožňuje v Inspektoru
# vybrat jinou scénu střely pro Ashe. Při spuštění scény, pokud je override nastaven,
# nahradí `bullet_scene` zděděnou z CharacterBase, aniž by se přepisovala ve skriptu.
# Pokud override zůstane prázdný (null), použije se výchozí střela z CharacterBase.
